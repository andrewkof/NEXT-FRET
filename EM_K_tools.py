...
def read_real_data(filename):
    """
    Read 'Time' and 'FRET' columns from a CSV produced by the spectro-scope
    and return them as 1-D NumPy arrays (or lists, if you prefer).
    Rows where either value is missing are discarded.
    """
    df = pd.read_csv(filename, usecols=[0, 1])
    df = df.dropna(how="any")
    t_data = df.iloc[:, 0].to_numpy()   # Time
    e_data = df.iloc[:, 1].to_numpy()   # FRET
